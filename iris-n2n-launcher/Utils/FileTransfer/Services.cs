...
public sealed class FileTransferService : IDisposable
{
    ...
    private static string SanitizeIncomingFileName(string fileName)
    {
        ...
        {
            throw new InvalidDataException("接收的文件名无效。");
        }

        if (sanitized.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)
        {
            throw new InvalidDataException("接收的文件名包含非法字符。");
        }
        ...
        {
            throw new InvalidDataException("接收的文件名属于系统保留名称。");
        }
    ...
    private static string GetValidatedSavePath(string savePath, string incomingFileName)
    {
        ...
        {
            throw new InvalidDataException("保存目录无效。");
        }
        ...
        {
            throw new InvalidDataException("检测到非法保存路径。");
        }
